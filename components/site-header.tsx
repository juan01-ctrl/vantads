...
export function SiteHeader() {
  ...
  const root = atRoot ? "" : "/";
  const links = linkKeys.map((l) => ({ ...l, href: `${root}${l.href}`, label: t(`nav.${l.label}`) }));
  useEffect(() => { const lock = open ? "hidden" : ""; document.body.style.overflow = lock; document.documentElement.style.overflow = lock; if (open) lenis?.stop(); else lenis?.start(); return () => { document.body.style.overflow = ""; document.documentElement.style.overflow = ""; lenis?.start(); }; }, [open, lenis]);
