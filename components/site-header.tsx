...
export function SiteHeader() {
  ...
  return <header className="absolute inset-x-0 top-0 z-20 px-5 py-6 sm:px-8 sm:py-7">
    <nav className="mx-auto flex max-w-[1038px] items-center justify-between" aria-label="Main navigation">
      ...
      <div className="flex items-center gap-4 md:hidden"><LangSwitcher /><button className="text-white" aria-label={open ? "Close menu" : "Open menu"} aria-expanded={open} onClick={() => setOpen(!open)}>{open ? <CloseIcon /> : <MenuIcon />}</button></div>
    </nav>
