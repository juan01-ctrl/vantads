...
export function ProjectCard({ project, className = "" }: { project: Project; className?: string }) {
  ...
  const enter = () => {
    setHovering(true);
    document.body.classList.add("is-media-hover");
    play();
  };
  ...
  const leave = () => {
    setHovering(false);
    setPlaying(false);
    document.body.classList.remove("is-media-hover");
    pause();
  };
  ...
  return <>
    <article ref={cardRef} className={`media-card group relative block w-full cursor-none overflow-hidden rounded-[16px] bg-[#161616] ${className}`} onMouseEnter={enter} onMouseLeave={leave} onMouseMove={onMove} onClick={open}>
      <video ref={videoRef} src={project.previewVideo} poster={project.poster} aria-label={alt} muted playsInline autoPlay={false} loop preload="none" className="absolute inset-0 h-full w-full object-cover" />
      ...
      <button ref={dotRef} type="button" onClick={open} className={`media-cursor pointer-events-none absolute left-0 top-0 z-20 flex h-20 w-20 items-center justify-center rounded-full opacity-0 transition-opacity duration-300 ${hovering ? "opacity-100" : ""}`} aria-hidden="true" tabIndex={-1}>
        <svg className="media-cursor-path" viewBox="0 0 80 80" fill="none" aria-hidden="true">
          ...
          <text className="media-cursor-text" textAnchor="middle" startOffset="50%">
            <textPath href={`#phone-cursor-arc-${project.title.replace(/\W/g, "-")}`} startOffset="50%">{t("modal.phoneView")}</textPath>
          </text>
        </svg>
        <PhoneIcon />
      </button>
      <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-4 p-5 sm:p-6">
        <div><p className="eyebrow mb-2 text-white/65">{category}</p><h3 className="max-w-[290px] text-[15px] leading-[1.15] text-white sm:text-[17px]">{project.title}</h3></div>
      </div>
    </article>
    <div className={isOpen ? "fixed inset-0 z-50 flex items-center justify-center bg-black/85 p-4 backdrop-blur-sm" : "hidden"} role="dialog" aria-modal="true" aria-label={t("modal.open")}>
      <button type="button" className="absolute inset-0 cursor-default" aria-label={t("modal.close")} onClick={close} />
      <div className="phone-modal relative z-10 h-[min(84vh,760px)] aspect-[9/16] overflow-hidden rounded-[22px] border border-white/20 bg-[#111] shadow-2xl shadow-black/70">
        <video ref={modalVideoRef} src={project.video} poster={project.poster} aria-label={alt} playsInline controls className="h-full w-full object-cover" />
        <button type="button" onClick={close} className="absolute right-3 top-3 flex h-9 w-9 items-center justify-center rounded-full border border-white/25 bg-black/50 text-lg text-white backdrop-blur-sm transition-colors hover:bg-white hover:text-black" aria-label={t("modal.close")}>×</button>
        <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-5 pt-14"><p className="eyebrow text-white/70">{category}</p><p className="mt-1 text-sm text-white">{project.title}</p></div>
      </div>
    </div>
  </>;
}
