...
import { ArrowIcon, BoltIcon, CartIcon, LayersIcon, MotionIcon, PlayIcon, SparklesIcon, VideoIcon } from "./icons";
...
function ServiceCard({ index, className = "", title, text, icon }: { index: string; className?: string; title: string; text: string; icon: ReactNode }) { return <article className={`service-card relative flex flex-col justify-between overflow-hidden border border-white/[.06] bg-[#121212] p-5 sm:p-6 ${className}`}><span className="service-index">{index}</span><div className="service-icon-wrap"><div className="service-icon flex h-9 w-9 items-center justify-center rounded-full border border-white/15 text-[#b65a70]">{icon}</div><span className="service-icon-line" /></div><div><h3 className="max-w-[170px] text-[18px] leading-[1.02] tracking-[-.04em] text-white">{title}</h3><p className="mt-3 max-w-[250px] text-[11px] leading-[1.45] text-[#85827f]">{text}</p></div></article>; }
