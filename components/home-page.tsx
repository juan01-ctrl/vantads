...
function ServiceCard({ index, title, text, icon: Icon }: { index: number; title: string; text: string; icon: (p: { className?: string }) => ReactNode }) {
  return <article className="service-card group relative flex min-h-[248px] flex-col justify-between overflow-hidden bg-[#0e0e0e] p-6 sm:p-7">
    <div className="flex items-start justify-between">
      <span className="font-mono text-[10px] tracking-[.14em] text-[#5c5a57]">{String(index + 1).padStart(2, "0")}</span>
      <Icon className="service-icon h-[26px] w-[26px] text-[#b65a70]" />
    </div>
    <div>
      <h3 className="text-[20px] leading-[1.05] tracking-[-.045em] text-white">{title}</h3>
      <p className="mt-3 max-w-[260px] text-[12px] leading-[1.55] text-[#8a8783]">{text}</p>
    </div>
    <span className="service-rule pointer-events-none absolute inset-x-0 bottom-0 h-px origin-left scale-x-0 bg-[#d34667]" aria-hidden="true" />
  </article>;
}
...
<section id="services" className="border-t border-white/[.07] px-5 py-20 sm:px-8 sm:py-28"><div className="grid gap-12 md:grid-cols-[1.1fr_2.9fr]"><div className="flex flex-col justify-between"><div><SectionLabel>{t("services.label")}</SectionLabel><h2 className="mt-6 text-[clamp(48px,7vw,88px)] font-light leading-[.85] tracking-[-.08em] text-white">{t("services.titleA")}<br />{t("services.titleB")}</h2></div><p className="eyebrow mt-12 max-w-[110px] whitespace-pre-line text-[#e2dfda]">{t("services.tagline")}</p></div><div className="grid gap-px overflow-hidden border border-white/[.08] bg-white/[.08] sm:grid-cols-2">{services.map((s, i) => <ServiceCard key={s.title} index={i} title={s.title} text={s.text} icon={serviceIcons[i % serviceIcons.length]} />)}</div></div></section>
    ...
    <section id="process" className="border-t border-white/[.07] px-5 py-20 sm:px-8 sm:py-28"><div className="grid gap-12 md:grid-cols-[1.1fr_2.9fr]"><div><SectionLabel>{t("process.label")}</SectionLabel><h2 className="mt-6 text-[clamp(48px,7vw,82px)] font-light leading-[.84] tracking-[-.08em] text-white">{t("process.titleA")}<br />{t("process.titleB")}<br /><span className="display-serif">{t("process.titleC")}</span></h2></div><div className="grid border-l border-t border-white/[.12] sm:grid-cols-2">{steps.map(([title, text], i) => <article key={i} className="process-step group relative flex min-h-[210px] flex-col border-b border-r border-white/[.12] p-6 sm:p-7"><div className="flex items-baseline gap-4"><span className="display-serif text-[52px] leading-none text-[#d34667]">0{i + 1}</span><span className="process-line h-px flex-1 origin-left bg-white/[.14] transition-transform duration-500 group-hover:scale-x-100 sm:scale-x-0" aria-hidden="true" /></div><div className="mt-auto pt-10"><h3 className="text-[19px] tracking-[-.04em] text-white">{title}</h3><p className="mt-3 max-w-[280px] text-[12px] leading-[1.55] text-[#8a8783]">{text}</p></div></article>)}</div></div></section>
